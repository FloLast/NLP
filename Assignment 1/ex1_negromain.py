...
sentence = text2sentences('moby_1000.txt')
sentence = [ x for x in sentence if len(x)>1]
X, Y, freq, all_words = sentences2input(sentence)
Y = sentence2output(sentence,Y)

#%%
def neural_net(X=X, Y=Y, neurons=50, epoch=5, learning_rate=0.5, all_words=all_words):
    ...
    # début de la boucle pour calculer la loss
        for row in tqdm(range(X_shape)):
            ...
            # création de w_2_ pour modifier w_2
            w_2_ = w_2
            
            '''NEGATIVE SAMPLING'''
            time2 = time.clock()
            
            # selection de 5 mots au hasard avec une proba de 0 + le mots le plus associé
            chosen_words = []
            while len(chosen_words)<5: #ajoute un nombre au hasard, verification de sa proba et s il est different du mot de la ligne, puis on l'ajoute
                rand = np.random.randint(0, all_words_len)
                if (Y.iloc[row,:][all_words[rand]] == 0)&(Y.columns[row]!=all_words[rand]):
        ...
        print('Forward  : mean '+str(np.mean(time12))+' / std : '+str(np.std(time12)))
